...
// =============================================================================
// DataLoader Interface Generation
// =============================================================================
...
pub fn generate_dataloaders(
    ...
    for proto_file in all_files {
        ...
        for message in &proto_file.message_type {
            ...
            if let Some(entity_opts) = get_cached_entity_options(proto_file_name, msg_name) {
                type_imports.insert(msg_name.to_string());

                // Generate {entity}ById loader
                loaders.push(DataLoaderDef {
                    name: format!("{}ById", msg_name.to_lower_camel_case()),
                    key_type: "number".to_string(),
                    value_type: format!("{} | null", msg_name),
                    description: format!("Load {} by primary key", msg_name),
                });

                // Generate loaders for has_many relations
                for relation in &entity_opts.relations {
                    if relation.r#type() == RelationType::HasMany {
                        let related_type = extract_type_name(&relation.related);
                        let fk_camel = relation.foreign_key.to_upper_camel_case();

                        loaders.push(DataLoaderDef {
                            name: format!("{}By{}", related_type.to_lower_camel_case() + "s", fk_camel),
                            key_type: "number".to_string(),
